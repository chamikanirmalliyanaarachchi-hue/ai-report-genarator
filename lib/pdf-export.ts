...
export async function downloadReportPdf(
  ...
  const drawSlimHeader = () => {
    ...
    doc.setFontSize(9);
    doc.text("AI Report Generator", margin, 7);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.text("AI Report Generator", margin, 13);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
...
function stampPdfFooter(
  ...
  for (let p = 1; p <= pages; p++) {
    ...
    doc.text("AI Report Generator · Confidential", margin, pageH - margin + 8);
    doc.text(
      `Page ${p} / ${pages}`,
      pageW - margin - 18,
      pageH - margin + 8
...
export async function downloadReportDocx(
  ...
  const children: any[] = [
    new Paragraph({
      ...
      children: [new TextRun({ text: "AI Report Generator", bold: true })],
    }),
    new Paragraph({
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Generated by AI Report Generator",
          italics: true,
          color: "999999",
        }),
      ],
